...
9. Each scoring period, users receive a new same-goal Period Match. If both meet their period commitments, both earn their own match rewards; if one misses, the committed player can complete an extra verified session to unlock the Make-Up Bonus.
...
GoGymGo should launch with one React Native and TypeScript codebase using Expo Router and Expo development builds for iOS and Android. Native Expo modules or narrowly scoped custom development-build modules can provide HealthKit, Health Connect, camera, local authentication, Bluetooth, push, and background capabilities without duplicating the entire product in Swift and Kotlin. Cash-prize verification still remains server-authoritative and must be proven on physical devices before launch.

The backend should use a TypeScript/NestJS modular monolith on managed Google Cloud infrastructure, PostgreSQL with PostGIS for user, profile, competition, ledger, payout, and regional data, object storage for optional avatar/photo media, Redis and BullMQ for read projections and retryable jobs, and Firebase Cloud Messaging for notifications. Hyperwallet Pay Portal should collect winner identity, tax, and bank-transfer details outside GoGymGo; the server creates payees and payments and reconciles webhooks.

The analytics layer should start with a first-party PostgreSQL event ledger and PostHog for internal product analytics. When sponsor volume requires warehouse reporting, stream normalized server events into Cloud Storage and BigQuery and expose sponsor-specific reporting through Looker Studio or a purpose-built portal. This keeps the MVP on one cloud while preserving auditable impressions, clicks, reach, frequency, and billing data.
...
- Sponsor-facing analytics begin with a first-party PostgreSQL event ledger and can graduate to Cloud Storage, BigQuery, and sponsor-specific Looker reporting without adding a second cloud.
