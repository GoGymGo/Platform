...
resource "aws_ecs_service" "worker" {
  cluster = aws_ecs_cluster.main.id
  # The worker owns a singleton database lease. At desired count one, ECS rounds
  # 101% down to one runnable task, so replacement remains stop-first while AZ
  # rebalancing still accepts the deployment configuration.
