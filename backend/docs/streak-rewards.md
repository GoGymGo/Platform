...
- Only `workout_sessions` rows with `status = 'verified'` earn streak credit.
...
`src/app.module.ts`. `streak-calculation.ts` is a pure, timezone-safe calendar
engine; `streaks.service.ts` owns the authenticated database query; and the DTO
and controller define the OpenAPI contract.
...
- `mobile-app/app/(tabs)/home/index.tsx` places the badge panel below the active
  workout card.
...
`mobile-app/.env.example`. Sign in with a Firebase account and open Home. The
...
- Unit tests cover empty, active, grace-period, duplicate-period, stale, invalid
  date, and cross-year Monday-week calculations.
