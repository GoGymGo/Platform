...
| Direct Weekly Challenges                              | `data/appData.ts`, `app/(tabs)/squad/index.tsx`                                          | exact-Contest match read plus `/v1/competitions/:monthKey/weekly-challenges/*`                    | Connected; only accepted, unblocked friends with active same-goal enrollment in the exact current regional Contest/week are eligible. Request create/respond/cancel is owned, idempotent, and explicit. A match requires accepted-request provenance and one database-constrained assignment per player/week. Partner reads contain only Alias, permitted streak projections/current/best streak, monthly verified-day total, and weekly verified count. Projected multiplier/entries are labeled provisional; only settled values are banked. No automatic stranger pairing or client-stored acceptance authority exists |
