...
3. Build and push `services/api/Dockerfile` from the repository root, then resolve the image to an Artifact Registry `@sha256:` digest.
...
Terraform owns resource topology, IAM, non-secret configuration, probes, and alerts. `.github/workflows/deployment.yml` owns image-only revisions and enforces this sequence:
