...
A fresh database starts with new payout releases paused. `HYPERWALLET_ENABLED=false` also fails before any claim transition or payment reservation, so a disabled environment cannot manufacture a false uncertain submission.
...
## Global release safety control

- Payout operators can read `GET /v1/operator/payout-release-control`. The same state is embedded in every claim review.
- Only an administrator can call `POST /v1/operator/payout-release-control/status-action`. The request requires `pause` or `resume`, the exact `expectedVersion`, an idempotency key, and a reason.
- Pause/resume and payment reservation lock the same singleton database row. A pause that commits first prevents a new reservation. A reservation that commits first is already in flight and must be reconciled rather than cancelled or recreated.
- Every successful state change increments the control version exactly once and records exact previous and next state in the append-only operator audit log. Database guards reject deletion, identity changes, same-state updates, missing administrator attribution, and invalid version increments.

Do not resume after an incident until provider connectivity, credentials, reconciliation queues, operator staffing, and the approved incident ticket have been checked. Resuming enables new reservations; it does not retry or replace an uncertain payment.
