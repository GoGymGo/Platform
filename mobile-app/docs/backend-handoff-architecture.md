...
Build the first production backend as a TypeScript modular monolith. Keep the Expo app as an untrusted client, Firebase Authentication as the identity provider, PostgreSQL/PostGIS as the system of record, Redis/BullMQ for short-lived jobs and ranking projections, and Hyperwallet Pay Portal as the hosted payout experience.
...
| Jobs/cache | Redis + BullMQ | Idempotent webhook follow-up, push jobs, retryable projections, and fast leaderboard read models. PostgreSQL remains authoritative. |
...
| `@nestjs/bullmq`, `bullmq`, `ioredis` | Retryable background work and read-model refreshes. |
| `pino` | Structured logs with mandatory redaction. |
| `@opentelemetry/*` | Trace API, SQL, queues, and external-provider calls. |
| `testcontainers` | Integration tests against real PostgreSQL/Redis containers. |

NestJS documents BullMQ queue integration and rate limiting at <https://docs.nestjs.com/techniques/queues> and <https://docs.nestjs.com/security/rate-limiting>.
...
| Leaderboards | PostgreSQL truth plus Redis read projections. |
...
1. Provision Firebase production apps, Cloud Run, Cloud SQL/PostGIS, Redis, secrets, object storage, and separate development/staging/production environments.
...
7. Add admin review tooling, privacy export/deletion, rules acceptance, observability, rate limits, and E2E coverage.
