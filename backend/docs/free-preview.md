...
Double-click `start-free-preview.cmd` in the repository root, or run:
...
6. writes the new URL to the ignored `mobile-app/.env.local`; and
...
- Never commit `backend/.env`, `mobile-app/.env.local`, Google credential
