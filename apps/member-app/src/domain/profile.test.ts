...
describe('public profile identity', () => {
  ...
  it('keeps a saved Alias editable while private identity presentation is active', () => {
    assert.deepEqual(
      publicIdentityFromAccountProfile({
        callsign: 'GG-ABC123',
        publicIdentityMode: 'private',
        publicName: null,
        screenName: 'MOVE_MORE',
        version: 3
      }),
      { callsign: 'GG-ABC123', displayName: 'MOVE_MORE', mode: 'private' }
    )
  })
