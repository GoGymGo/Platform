...
$mobileDirectory = Join-Path $projectRoot "mobile-app"
$backendEnvironment = Join-Path $backendDirectory ".env"
$composeBase = Join-Path $backendDirectory "compose.yml"
$composePreview = Join-Path $backendDirectory "compose.free-preview.yml"
$mobileEnvironment = Join-Path $mobileDirectory ".env.local"
$expoCli = Join-Path $mobileDirectory "node_modules\expo\bin\cli"
...
if (-not (Test-Path -LiteralPath $backendEnvironment)) {
  throw "backend/.env is missing. Create it from backend/.env.example first."
}
