...
- Welcome leads with the GoGymGo logo followed immediately by one compact three-step strip: Show Up, Prove It, and Win. The strip uses the standard cyan/green active treatment so the loop reads as one process. Pink on the root screen is reserved for the center logo word, sponsor marks, prize-pool, and prize draw/reward accents. The signup reward reads `ON SIGNUP YOU RECEIVE A`, `FREE ENTRY`, and `INTO THE MONTHLY PRIZE DRAW`; the payout line reads `15% OF PLAYERS GET PAID`. The panel also contains the projected prize pool and a text-only `MONTH SPONSOR` card; that compact card does not repeat the sponsor mark already shown in the page-level sponsor rail. Registration-window detail belongs on Commitment. Page one contains only `CREATE ACCOUNT` and, in local development builds, `PREVIEW APP FLOW`. `CREATE ACCOUNT` opens page two at `/join` instead of placing every application path on the welcome screen.
...
- Sponsor application is a dedicated public route at `/sponsor/apply`, also reachable from Profile. The frontend collects company name, work email, and target region, validates them locally, and records a local application draft until the campaign backend is connected. The page explains regional targeting, app-wide placements, and the configurable per-verified-user sponsorship target.
- Gym registration is a dedicated public route at `/gym/register`. A gym manager requests GoGymGo entry and exit QR codes for one physical location by providing gym name, manager name, work email, street address, and city/region. The frontend validates and stores a local request draft. Submitting a request never issues or activates a code automatically: GoGymGo must verify the manager and location before creating the partner-gym record and issuing checkpoint-specific QR codes.
...
- The `REGIONAL RANKINGS` screen opens by explaining that Category Score determines rank while Prize Draw Entries determine draw odds. Before scoring starts, `YOUR STANDING` says rankings open after Scoring Week 1 and does not show a misleading zero score or rank. After Week 1 settles, Category Score updates cumulatively after every scoring week and includes each settled week's `1X`, `2X`, or `3X` Period Match result. One compact standing panel otherwise shows the user's Weekly Goal category, `PENDING` before settlement or `SYNCING` while a live rank is unavailable, and Category Score without repeating streak or Prize Draw Entry metrics from Home. A single 1-7 segmented control replaces seven leader-preview cards and defaults to the user's hydrated Weekly Goal. Selecting a category updates one mobile-readable Top 10 list showing rank, public Alias, Category Score, and pink `3X`, `2X`, or `1.5X` badges beside the current Top Three Category Finishers. Demo standings are hidden by default in every competition phase and appear only after the explicit `PREVIEW SAMPLE RANKINGS` action; they remain labeled `PREVIEW TOP 10` and never mix silently with live standing state. Detailed weekly-update and tie-break copy stays collapsed behind `HOW RANKING WORKS`.
- The Winners Circle is a dedicated previous-month results page available from Regional Ranks. On the first day of each month in the selected competition-region timezone, it appears once after a returning user successfully signs in only when audited, settled results are available. Dismissing it stores that account's current login month so it does not interrupt the same account again that day or month. New users still complete onboarding first. If settlement data is unavailable, the screen shows `RESULTS NOT AVAILABLE YET` and never renders preview aliases or payouts as real results. Local preview mode may show clearly labeled `SAMPLE RESULTS`. A two-tab control separates `CATEGORY CHAMPIONS` from `PRIZE DRAW WINNERS` so the page never presents both long lists at once. Category champions are not described as cash winners unless they were also selected by the weighted prize draw.
...
- All frontend sample opponents, category standings, creator workouts, completed results, payout winners, and session telemetry are accessed through one explicit app-data source. Local preview mode may expose them only with `DEMO` or `SAMPLE` labeling. A production build with no connected backend returns honest empty or unavailable states. Production screens must never import mock fixtures directly or display fabricated values as live user, competition, health, or payout data.
...
1. User installs or opens GoGymGo and first sees the GoGymGo logo welcome page with only CREATE ACCOUNT and the development-only PREVIEW APP FLOW action.
...
- Expo Location for one-time foreground device coordinates, plus a backend service-area resolver using PostGIS polygons or equivalent versioned geographic boundaries. Google Maps Geocoding or Mapbox Geocoding can support postal/address confirmation; the client must not make the final eligibility assignment. The current frontend preview uses conservative metro service areas and Canadian postal-area mappings behind a tested domain boundary so the production backend can replace them without changing screen behavior.
