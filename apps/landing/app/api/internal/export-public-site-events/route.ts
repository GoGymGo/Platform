...
import { exportPublicSiteEvents } from "@/app/api/public-site-storage";
...
export async function GET(request: Request) {
  ...
  const page = readEventExportPage(request);
  if (!page) {
    return ownerError("Use a valid export limit and cursor.", 400);
  }

  try {
    const exported = await exportPublicSiteEvents(
      getD1(),
      page,
      new Date(),
      retention.eventDays,
    ...
    return new Response(
      ...
      {
        headers: exportHeaders("landing-public-site-events.json"),
        status: 200,
      },
    );
  } catch {
    return ownerError(
      "The public-site event export is temporarily unavailable.",
      503,
    );
  }
}
